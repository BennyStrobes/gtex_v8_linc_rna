...
def extract_previously_used_variants(previous_variant_bed_file):
	used_variants = {}
	f = open(previous_variant_bed_file)
	for line in f:
		line = line.rstrip()
		used_variants[line] = 1
	f.close()
	return used_variants

# Add RV calls to cluster_struct object
def extract_rare_variants(variant_bed_file, previous_variant_bed_file, cluster_struct, individuals):
	# Check if first window. If so, no filter
	if variant_bed_file == previous_variant_bed_file:
		used_variants = {}  # empty
	else:
		used_variants = extract_previously_used_variants(previous_variant_bed_file)
	# Stream variant file
	f = open(variant_bed_file)
	for line in f:
		line = line.rstrip()
		if line in used_variants:
			continue
		data = line.split()
...
def enrichment_analysis(namer, cluster_struct, output_handle):
	...
	constant = 1.0
	# Compute odds ratios
	num_frac = (constant + float(a))/(constant + float(b))
	den_frac = (constant + float(c))/(constant + float(d))
...
for pvalue_threshold in pvalue_thresholds:
	...
	# Add RV calls to cluster_struct object
	cluster_struct = extract_rare_variants(variant_bed_file, previous_variant_bed_file, cluster_struct, individuals)
